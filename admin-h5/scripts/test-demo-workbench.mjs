...
const source = readFileSync(path.join(root, 'src/views/DemoWorkbench.vue'), 'utf8')
assert.match(source, /正在准备本次体验/)
assert.match(source, /请用另一台手机扫描/)
assert.match(source, /订单可能不是最新/)
assert.match(source, /重新开始30分钟体验/)
assert.match(source, /体验码无效或已过期，请重新扫描体验卡/)
assert.match(source, /暂时无法连接体验服务，请检查网络后重试/)
assert.doesNotMatch(source, /errorMessage\.value\s*=\s*error\.response\?\.data\?\.msg\s*\|\|\s*error\.message/)
assert.match(source, /customerCodeImageUrl/)
assert.match(source, /nextDemoAction/)
assert.match(source, /setInterval\(refreshSnapshot, 2000\)/)
