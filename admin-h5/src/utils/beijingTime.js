...
// 'YYYY年M月D日 HH:mm'（看板/详情页用的完整中文时间）
export function formatBeijingLong(value) {
  const date = parseServerTime(value)
  if (!date) return ''
  const { year, month, day, hour, minute } = beijingParts(date)
  return `${year}年${month}月${day}日 ${pad(hour)}:${pad(minute)}`
}

// 分钟差（now - value），按真实时刻算，不受时区影响；解析失败返回 null
...
// value 是否落在"北京时间的今天"
export function isBeijingToday(value, now = new Date()) {
  const date = parseServerTime(value)
  if (!date) return false
  const t = beijingParts(date)
  const d = beijingParts(now)
  return t.year === d.year && t.month === d.month && t.day === d.day
}

// value 是否落在"北京时间的本月"
export function isBeijingThisMonth(value, now = new Date()) {
  const date = parseServerTime(value)
  if (!date) return false
  const t = beijingParts(date)
  const d = beijingParts(now)
