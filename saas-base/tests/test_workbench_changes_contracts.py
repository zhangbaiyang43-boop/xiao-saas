...
class WorkbenchCursorContractTest(unittest.TestCase):
    ...
    def test_visibility_waiter_kitchen(self):
        ...
        # Waiter R2: only done + unserved
        self.assertFalse(wss.is_order_visible_in_workbench(pending, "waiter"))
        self.assertFalse(wss.is_order_visible_in_workbench(preparing, "waiter"))
        self.assertTrue(wss.is_order_visible_in_workbench(done, "waiter"))
        served = SimpleNamespace(status="done", served_at="x")
        self.assertFalse(wss.is_order_visible_in_workbench(served, "waiter"))
