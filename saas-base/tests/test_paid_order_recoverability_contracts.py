...
class PaidOrderRecoverabilityContractsTest(unittest.TestCase):
    def test_order_is_created_before_payment_request(self):
        ...
        self.assertIn('status="pending_payment"', create_order_source)
    ...
    def test_consumer_order_query_recovers_paid_pending_payment_order(self):
        get_my_order_source = function_source("get_my_order")
        self.assertIn("_recover_wxpay_order_if_paid(order, db)", get_my_order_source)
    ...
    def test_merchant_order_query_recovers_paid_pending_payment_orders(self):
        list_orders_source = function_source("list_orders")
        self.assertIn('if order.status == "pending_payment"', list_orders_source)
        self.assertIn("_recover_wxpay_order_if_paid(order, db)", list_orders_source)
        self.assertIn("await db.commit()", list_orders_source)
