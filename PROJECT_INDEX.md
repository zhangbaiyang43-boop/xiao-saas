...
- 商家后台前端系统 Phase-03B：Dashboard 经营视图 Touch And Migrate（只读，验收证据）：[docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE03B_DASHBOARD_MIGRATION.md](./docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE03B_DASHBOARD_MIGRATION.md)
- 商家后台前端系统 Phase-03C：菜品管理（MenuManage.vue）状态真实性 Touch And Migrate（只读，验收证据）：[docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE03C_DISH_STATE_MIGRATION.md](./docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE03C_DISH_STATE_MIGRATION.md)
...
- 商家后台前端系统 Phase-05C：CustomerList 工作上下文保持，Phase-05（高频任务效率基线）收口（只读，验收证据）：[docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE05C_CUSTOMER_CONTEXT_PRESERVATION.md](./docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE05C_CUSTOMER_CONTEXT_PRESERVATION.md)
- **商家后台前端系统 Phase-06：下一优先级审计已完成——SECURITY_GATE=PASS，四维度体验审计（信息层级/响应式/视觉/可用性）冻结 TOP_PRIORITY_AREA=VISUAL_SYSTEM（深色模式 token 合规缺口，PageHeader 等 20 页共用组件+多个高频页硬编码颜色），定义待实施的 Phase-06A**：[docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE06_NEXT_PRIORITY_AUDIT.md](./docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE06_NEXT_PRIORITY_AUDIT.md)
- **商家后台前端系统 Phase-06-SEC：CustomerList 会话上下文凭证泄露修复——identity 改为只用 tenant_id，登出时显式清理 sessionStorage，已实施已验证**：[docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE06_SEC_CUSTOMER_CONTEXT_CREDENTIAL_REMEDIATION.md](./docs/frontend/ADMIN_FRONTEND_SYSTEM_PHASE06_SEC_CUSTOMER_CONTEXT_CREDENTIAL_REMEDIATION.md)
